...
export async function GET(request: Request) {
  ...
  const { data: solicitud, error } = await sesion.supabase
    .from("solicitudes_credencial_pdf")
    .select("estado, pdf_path, codigo_documento")
    .eq("matriculado_id", sesion.matriculadoId)
    .maybeSingle()

  if (error || !solicitud || solicitud.estado !== "disponible" || !solicitud.pdf_path) {
    return NextResponse.json(
      { ok: false, mensaje: "No hay un PDF disponible para descargar." },
      { status: 404 },
    )
  }

  const { data: archivo, error: errorDescarga } = await sesion.supabase.storage
    .from("credenciales-pdf")
    .download(solicitud.pdf_path)

  if (errorDescarga || !archivo) {
    console.error("Error descargando PDF privado:", errorDescarga)
    return NextResponse.json(
      { ok: false, mensaje: "No se pudo descargar el PDF." },
      { status: 500 },
    )
  }
  ...
  const { error: errorEntrega } = await sesion.supabase
    .from("solicitudes_credencial_pdf")
    .update({ estado: "entregada", entregado_en: ahora, updated_at: ahora })
    .eq("matriculado_id", sesion.matriculadoId)
    .eq("estado", "disponible")

  if (errorEntrega) {
    return NextResponse.json(
      { ok: false, mensaje: "No se pudo registrar la entrega del PDF." },
      { status: 500 },
    )
  }

  const { error: errorBorrado } = await sesion.supabase.storage
    .from("credenciales-pdf")
    .remove([solicitud.pdf_path])

  if (errorBorrado) {
    console.error("No se pudo borrar el PDF ya entregado:", errorBorrado)
  }

  const nombre = `Credencial-RENACLI-${solicitud.codigo_documento || "tecnico"}.pdf`
  ...
  return new NextResponse(bytes, {
    ...
    headers: {
      ...
      "X-RENACLI-Document-Code": solicitud.codigo_documento || "",
    },
  })
}
